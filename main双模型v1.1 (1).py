...
class ArknightsApp:
    def __init__(self, root):
        ...
        # Canvas & Scrollbars
        self.history_canvas  = tk.Canvas(self.history_container, bg="white", width=550)
        ...
        self.models = {
            "实验模型": self.load_single_model("models/best_model_full.pth"),
            "对照模型": self.load_single_model("models/backup_model.pth"),
        }
        self.model_errors = {name: 0 for name in self.models}

    def load_single_model(self, path):
    ...
    def create_widgets(self):
        ...
        tk.Label(self.result_frame,
            ...
            .pack(pady=(5, 0))  # pack 默认水平居中

        self.result_label_main = tk.Label(self.result_frame,
                                          text="", font=("Helvetica", 12))
        self.result_label_main.pack(pady=(0, 5))
        # 预测2
        tk.Label(self.result_frame,
                 text="预测结果（对照模型）:",
                 font=("Helvetica", 12, "bold")) \
        ...
        self.result_label_backup = tk.Label(self.result_frame,
                                            text="", font=("Helvetica", 12))
        self.result_label_backup.pack(pady=(0, 5))
        ...
        tk.Button(fill_buttons_frame,
                  text="左边赢",
                  command=self.fill_left,
                  width=8,
                  bg="#E23F25",
                  fg="white",
                  activebackground="#C42F1C"
                  ).pack(side=tk.LEFT, padx=5)

        tk.Button(fill_buttons_frame,
                  text="右边赢",
                  command=self.fill_right,
                  width=8,
                  bg="#25ace2",
                  fg="white",
                  activebackground="#1E8CBF"
                  ).pack(side=tk.LEFT, padx=5)
        ...
        self.result_label = self.result_label_main
    ...
    def fill_left(self):
        actual = 'L'
        self.current_predictions = self.get_prediction()
        self.fill_data(actual)
        self.total_fill_count += 1

        for name, p in self.current_predictions.items():
            pred = 'R' if p > 0.5 else 'L'
            if pred != actual:
                self.model_errors[name] += 1
    ...
    def predict_with(self, model, left_signs, left_counts, right_signs, right_counts):
        with torch.no_grad():
    ...
    def get_prediction(self):
        if not hasattr(self, 'models') or not self.models:
            raise RuntimeError("没有任何模型被正确初始化")
        for name, m in self.models.items():
            if m is None:
                raise RuntimeError(f"模型 “{name}” 未正确初始化")
            ...
            for model_name, model in self.models.items():
                try:
                    ...
                    messagebox.showerror("错误", f"{model_name} 文件未找到，请检查路径或先训练模型")
                    p = 0.5
                except RuntimeError as e:
                    messagebox.showerror("错误", f"{model_name} 推理失败：{e}")
                    p = 0.5
                except ValueError:
                    messagebox.showerror("错误", f"{model_name} 输入数据格式有误")
                    p = 0.5
                except Exception as e:
                    messagebox.showerror("错误", f"{model_name} 预测时发生未知错误：{e}")
                    p = 0.5
    ...
    def predictText(self, prediction):
        if isinstance(prediction, dict):
            ...
            p_main = prediction.get("实验模型", 0.5)
            p_backup = prediction.get("对照模型", 0.5)

            self.result_label_main.config(
                text=f"实验模型： 左 {(1 - p_main):.2%} ／ 右 {p_main:.2%}"
            )
            self.result_label_backup.config(
                text=f"对照模型： 左 {(1 - p_backup):.2%} ／ 右 {p_backup:.2%}"
            )

            for lbl, p in [(self.result_label_main, p_main),
                           (self.result_label_backup, p_backup)]:
                left, right = 1 - p, p
                if left > 0.7:
                    fg = "#E23F25"
                elif right > 0.7:
                    ...
                    fg = "black"
                lbl.config(fg=fg, font=("Helvetica", 12, "bold"))
            return
    ...
    def auto_fetch_data(self):
        relative_points = [
            (0.9297, 0.8833),  # [0] 右ALL / 返回主页 / 加入赛事 / 开始游戏
            (0.0713, 0.8833),  # [1] 左ALL
            (0.8281, 0.8833),  # [2] 右礼物 / 自娱自乐
            (0.1640, 0.8833),  # [3] 左礼物
            (0.4979, 0.6324),  # [4] 本轮观望
        ]
        ...
        raw = loadData.match_images(screenshot, loadData.process_images)
        results = sorted(raw, key=lambda x: x[1], reverse=True)
        ...
        for idx, score in results:
            ...
            if idx == 0:
                ...
                # 投资逻辑
                if self.is_invest.get():
                    self.current_predictions = self.get_prediction()
                    p_main = self.current_predictions.get("实验模型", 0.5)
                    if p_main > 0.5:
                        pt = relative_points[0] if idx == 4 else relative_points[2]
                        loadData.click(pt);
                        print("投资右")
                    else:
                        pt = relative_points[1] if idx == 4 else relative_points[3]
                        loadData.click(pt);
                        print("投资左")
                    if self.game_mode.get() == "30人":
                        time.sleep(20)
    ...
    def update_statistics(self):
        ...
        base = (f"总填写次数: {self.total_fill_count} ，"
                f"全局错误: {self.incorrect_fill_count} ，"
                f"运行时长: {int(h)}h{int(m)}m")

        # 拼接各模型的错误数
        per_model = "  ".join(
            f"{name}错: {cnt}"
            for name, cnt in self.model_errors.items()
        )
        self.stats_label.config(text=f"{base}\n{per_model}")
